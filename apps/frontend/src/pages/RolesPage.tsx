 ...
 *
 * Roles admin (list). Create/edit + permission assignment happen on a dedicated
 * page (/admin/roles/new, /admin/roles/:id/edit) with a tabbed permission picker —
 * see RoleEditorPage. Protected roles (isSystem) are read-only.
 */
...
export function RolesPage() {
  ...
  const { loading, error, roles, permissions, deleteRole } = useRoles();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canWrite ? (
          <button type="button" onClick={() => navigate("/admin/roles/new")} className={primaryBtn}>New role</button>
        ) : null}
      </div>
      ...
      <section className="overflow-hidden rounded-lg border border-slate-800 bg-slate-900/40">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            {roles.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-6 text-slate-500">No roles yet.</td></tr>
            ) : (
              roles.map((r) => {
                const all = r.permissions.length >= totalPerms && totalPerms > 0;
                return (
                  <tr key={r.id} className="text-slate-200">
                    <td className="px-4 py-3">{r.name}{r.isSystem ? <SystemTag /> : null}</td>
                    <td className="px-4 py-3 text-slate-400">{r.description ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-400">{all ? "All" : `${r.permissions.length} / ${totalPerms}`}</td>
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        {canWrite ? (
                          <button type="button" disabled={r.isSystem} title={r.isSystem ? "Protected system role" : undefined} onClick={() => navigate(`/admin/roles/${r.id}/edit`)} className={rowBtn}>Edit</button>
                        ) : null}
                        {canWrite ? (
                          <button type="button" title="Create a new editable role from this one" onClick={() => navigate(`/admin/roles/new?from=${r.id}`)} className={rowBtn}>Clone</button>
                        ) : null}
                        {canDelete ? (
                          <button type="button" disabled={r.isSystem} title={r.isSystem ? "Protected system role" : undefined}
                            onClick={() => void run(async () => {
                              if (await confirm({ title: "Delete role", message: `Delete role "${r.name}"? This cannot be undone.`, confirmLabel: "Delete" }))
